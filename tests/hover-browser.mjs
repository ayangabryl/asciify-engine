...
try{
 ...
 for (const fallback of [false,true]) {
  ...
  await page.evaluate(()=>hover.update({motion:'caustics'}));await delay(200);const motion=await page.evaluate(()=>fingerprint());await delay(200);assert.notEqual(await page.evaluate(()=>fingerprint()),motion,'living image must change');
